...
The `session_id` used for the seven-tool API is the **bare Claude Code
session UUID**. (Earlier drafts wrapped the UUID in a `cc-<...>` source-
namespace tag, but the UUID itself is already globally unique so the
prefix was retired — see the `migrate_legacy_session_ids` migrator
which rewrites pre-existing `cc-` records on startup.)
...
6. **Session id namespacing.** *(Resolved post-draft — see WAL migrator
   `migrate_legacy_session_ids`.)* The bare Claude Code UUID is now the
   canonical `session_id`. The earlier `cc-<8-char-uuid>` / `cc-<full-uuid>`
   forms have been retired: the UUID is already globally unique and the
   prefix added no information.
